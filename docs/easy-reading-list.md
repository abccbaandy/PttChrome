...
16. **last-read 高亮＝title-match（pttbbs 真實邏輯）＋normalize-on-store＋decorate-on-render**（2026-07-18 重寫）：真實邏輯在 `3rd_script/pttbbs/mbbsd/bbs.c` `readdoent:830`——`strcmp(currtitle, subject_ex(title))==0` 的**每一列**都塗 `1;3c`（c 依該列自身 title_type：`□`=1紅 `R:`=3黃 `轉`=6青 `鎖`=5紫 `ˇ`=2綠），範圍 mark→行尾、**不含 author 欄**；author 亮白 `1;37`＝`isonline`（作者在線，bbs.c:815-823），與 last-read 無關；currtitle per-login 全域、讀完文即設 subject（bbs.c:2424）。→ **同主題多列同亮是正常行為**（實錄 20260717-224420 t=1937：296/298 同紅），單一列號游標模型（v1 `_lastReadNum`）必然殘紅。現行模型：map 永存去色列（`normalizeLastReadListRow`；**雙豁免**＝[8,12) 推文數欄 ＋ [17,29) 作者欄）；session 記 **`_lastReadTitle`**（`subjectOfListRow`＝title 區去 mark＋防禦性 loop 剝 `Re:`/`Fw:`，＝pttbbs `subject_ex` 等價）；教學雙路——frame-taught（`isLastReadStyledListRow` 命中 fg∈{1,2,3,5,6}→`noteLastRead(subject)`；漏抓 fail-safe）＋**主動教學**（`_beginOpen`/`_beginOpenPinned` 開文成功 onDone 直教，堵 partial 幀無樣式列的洞）；render（`buildListWindowLines`）對 subject 命中的**每列** clone 重上 `paintLastReadListRow`（色＝`listRowMarkFg` 由該列自身 mark 推，author 欄不動；subject 以 `row._subject` memoize）。生命週期：只有 cleanup 歸零；seed/rebuild/resume 一律保留（currtitle 全域、title 與序號空間無關，新幀自動重教）。**作者欄豁免＝2026-07-25 修復點**：normalize 原本只豁免推文數欄、把作者欄一併壓回預設，而 `paintLastReadListRow` 只重畫 [29,) → 進一篇「作者在線上」的文章再退回列表，該列 isonline 亮白被抹掉且不再還原（每次重繪再抹一次）＝作者永久變灰；錄製 20260725-153131 t=2869 證實 server 仍送 `ESC[1;37m<author>`。守護：`list_accumulate.test.js`（殘留＋欄位豁免＋●共存＋同主題多列紅黃並亮＋換篇退色＋isonline 不誤觸＋**isonline＋last-read 同列作者亮白保留（accumulate/游標實況/render 三面）**＋subject/markFg 純函式）、`list_session.test.js` 生命週期。
