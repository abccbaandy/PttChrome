...
- demand：視窗頂/底距 buffer 邊 **< bodyRows（一頁）** 即補（方向性、chain 不跨來源 fill/key），下一次 PgUp 永遠本地即時。到邊等待＝視窗 clamp、鍵 no-op（體感＝native 等 server）。
...
5c. **預讀＝錨定命令對**（jump 到 `bufferEdgeNum(方向)` → PgUp/PgDn；cursor 越過錨=新頁、等於=到邊）。錨定失敗 onFail flush。
