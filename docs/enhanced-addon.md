...
- **打字游標的座標系是格線，畫面的座標系會捲動 —— 兩者必須顯式對齊**。`#cursor` 是 `#BBSWindow`(`position:fixed`) 下的**絕對定位**元素，位置由 `term_view.convertMN2XYEx`（`firstGridOffset` + `chh`）算出，**完全不受 `.main` 的 `scrollTop` 影響**；而它要貼齊的那一列在 `#mainContainer` 裡，會跟著 `.main` 捲。只要 `.main` 有任何殘留捲動（多半來自沒清乾淨的 `paddingBottom`），游標就掉出該列。2026-08-20 的實例：好讀模式按 `X` 推文（functionMode 原生鏡像）時，`hideEasyReadingOverlaysKeepPage` 沒清掉 `accumulatePageLines` 留的 `paddingBottom:1em` ⇒ `.main` 還有 `chh-10` px 可捲、滾輪一碰輸入列就上移 ⇒ **閃爍游標戳出反白輸入匡**；不特定文章、離開文章才自動修好。修法兩層：清 padding（消除觸發條件）＋ `updateCursorPos` 在固定格線渲染的幀（`view._gridRender`）扣掉 `mainDisplay.scrollTop`（結構性守門，不再靠「每條模式切換路徑都記得清」）。詳見 `docs/easy-reading.md` functionMode 段，守護 `tests/e2e/offline/cursor_shape.offline.spec.js`。
