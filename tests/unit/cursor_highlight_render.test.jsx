...
// 實際上色的是 Row / LinkSegmentBuilder 硬寫的 `cx({ b2: highlighted })`。
// 這裡鎖住「Row 收到什麼 class 就掛什麼 class」，而不是鎖死某個顏色。
...
describe("Row 游標底色 class", () => {
  test("highlightClass 直接掛到 bbsline span（不再硬寫 b2）", () => {
    const { container } = render(
      <Row chars={chars("hello world")} row={3} highlightClass="b4" />
    );
  ...
  test("預設綠 b2 仍照常運作", () => {
    const { container } = render(
      <Row chars={chars("hello world")} row={3} highlightClass="b2" />
    );
    expect(bbsline(container).classList.contains("b2")).toBe(true);
  });

  test("沒帶 highlightClass 的列不上任何底色", () => {
    const { container } = render(<Row chars={chars("hello world")} row={3} />);
    const line = bbsline(container);
