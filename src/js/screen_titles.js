...
//
// pttbbs 的標題列有三種形狀（`mbbsd/vtuikit.c`）：
//   三段式 vs_header(title, mid, right)  →  `【title】` + 中段 + 右段
//                                            VMSG_HEADER_PREFIX/POSTFIX = 【 / 】
//   單段式 vs_hdr(title)                 →  `【 title 】`
//                                            VMSG_HDR_PREFIX/POSTFIX = "【 " / " 】"
//   兩段式 vs_hdr2bar(left, right)       →  左分類 + 右說明列
// 本專案用來判畫面的四個標題（主功能表／分類看板／精華文章／看板列表）**全部走
// 三段式**（menu.c#showtitle → vs_header），所以現行字面值是 `【X】`。
//
// 為什麼不直接比字面值：PTT 2026-09-20 公告「介面調整: 標題列與主選單底部狀態列
// 改版」（PTT2 09/20、PTT1 10/04）動了另外兩種形狀的括號 ——
//   單段式：`【 標題 】` → `【標題】`（移除內側空白）
//   兩段式：移除左半部的【】，改為「前後各留一格半形空白」
// 三段式公告說沒有變動，而且第 1 點還特地保證
//   「頂端 Row 0 左側仍維持「【主功能表】」不變」
// ⇒ **這次不會壞**。但既然括號會動，就把判定收斂成一個地方、同時吃兩種形狀，
// 下次再動時不必再翻八個檔案。公告第 4 點也提醒「日後改變的修改不會預先公告」。
//
// 兩種形狀：
//   `【X】…`（現行三段式）
//   ` X …`  （公告描述的去括號兩段式：前後各一格半形空白）
